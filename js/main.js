  ...
  if (!yaObtuvoCenDis) {
    ...
    for (var i = 0; i < feat.length; i++) {
      if (feat[i].l.BoroCD%100 < 19) {
        ...
        if(feat[i].j.j.length == 1){
          for (k = 0; k < feat[i].j.j[0].j.length; k++) {
            bounds.extend(feat[i].j.j[0].j[k]);
          }
        }else {
          for (var u = 0; u < feat[i].j.j.length; u++) {
            for (k = 0; k < feat[i].j.j[u].j[0].j.length; k++) {
              bounds.extend(feat[i].j.j[u].j[0].j[k]);
            }
          }
        }
        bcdCenter.push([feat[i].l.BoroCD,bounds.getCenter()]);
        /*
        var marcador = new google.maps.Marker({
        ...
        infoDis.push([boroName(parseInt(feat[i].l.BoroCD/100)),feat[i].l.BoroCD%100,distancia,bounds.getCenter(),feat[i].l.BoroCD]);
      }
    }
    yaObtuvoCenDis = true;
    //marksDis();
